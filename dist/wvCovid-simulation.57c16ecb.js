...
},{"topojson-client":"../node_modules/topojson/node_modules/topojson-client/index.js","topojson-server":"../node_modules/topojson/node_modules/topojson-server/index.js","topojson-simplify":"../node_modules/topojson/node_modules/topojson-simplify/index.js"}],"../node_modules/Scrubber/index.js":[function(require,module,exports) {
...
},{}],"data/westvirginiaCounties.json":[function(require,module,exports) {
module.exports="/westvirginiaCounties.b88f665c.json";
},{}],"scripts/wvCovid-simulation.js":[function(require,module,exports) {
"use strict";
...
var svg = d3.select('#wv-covid-simulation').attr('viewBox', [0, 0, width, height]).append('svg') //   .style('background-color', 'steelblue')
.attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')'); //////////////////////////////////////////////////////////////// TOOLTIP////////////////////////////////

var tooltip = d3.select('body').append('div').attr('id', 'tooltip').style('visibility', 'hidden');

function mouseOver(d, i) {
  // add custom tooltip html
  tooltip.attr('data-html', 'true').style('visibility', 'visible').html("<div class='row'><b>".concat(d.properties.NAME20, " County</div>"));
...
function mouseMove(d, widthEl) {
  ...
  var offset = d3.select('#wv-covid-simulation').node().getBoundingClientRect().x;
...
var colors = ['#e6e6e6', '#f2df91', '#f9c467', '#ffa93e', '#ff8b25', '#fd6a0b', '#f04f08', '#d8382e', '#c62832', '#af1c43', '#8a1739', '#701547']; // Geo Scale//

var projection = d3.geoMercator();
var path = d3.geoPath().projection(projection);
var colorScale = d3.scaleLinear().range(['#f1f1f1', 'green']);
var index = 0;
var delay = 1;
Promise.all([d3.json(require("/data/westvirginiaCounties.json")), d3.csv("https://raw.githubusercontent.com/nytimes/covid-19-data/master/us-counties.csv")]).then(ready).catch(function (err) {
...
function ready(_ref) {
  ...
  raw = raw.filter(function (d) {
    return d.state == 'West Virginia';
  });
  raw.map(function (d) {
    return d.datetime = parseTime(d.date);
  });

  var dates = _toConsumableArray(new Set(raw.map(function (d) {
    return d.date;
  })));

  colorScale.domain([0, d3.max(raw, function (d) {
    return +d.cases;
  })]); ///json file///

  var counties = topojson.feature(json, json.objects.westvirginiaCounties);
  projection.fitSize([width, height], counties);
  var map = svg.append('g').selectAll('path').data(counties.features).join('path').attr('d', path).attr('class', 'county').style('stroke', 'lightgrey').style('stroke-width', 0.1).on('mouseover', function (d) {
    return mouseOver(d);
  ...
  var getFilteredData = function getFilteredData() {
    var filtered = _toConsumableArray(raw).filter(function (d) {
      return d.date === dates[index];
    });

    var totalsMap = new Map(filtered.map(function (d) {
      return [d.fips, d];
    }));
    return totalsMap;
  };
  ...
  function casesMap() {
    map.transition().attr('fill', function (d) {
      return getFilteredData().get(d.properties.GEOID20) ? d3.interpolateReds(Math.log(+getFilteredData().get(d.properties.GEOID20).cases) / Math.log(10) / 6) : 'white';
    });
  }

  casesMap(); // Make a new scrubber

  var scrubber = new ScrubberView();
  document.querySelector('#scrubber-wrapper').appendChild(scrubber.elt);

  scrubber.onValueChanged = function (idx) {
    index = idx;
    casesMap();
  ...
  d3.select('#deaths-layer').on('click', function () {
    ...
    function deathsMap() {
      map.transition().attr('fill', function (d) {
        return getFilteredData().get(d.properties.GEOID20) ? d3.interpolateBlues(Math.log(+getFilteredData().get(d.properties.GEOID20).deaths) / Math.log(10) / 5) : 'white';
      });
    }

    deathsMap(); // Make a new scrubber
    // document.querySelector('#scrubber-wrapper').appendChild(scrubber.elt)
  ...
  var iterateIndex = null;

  function timer() {
    if (index <= dates.length - 1) {
      index = index + 1;
      scrubber.value(index); // casesMap()
  ...
  function render() {
    ...
    var newHeight = svgHeight - margin.top - margin.bottom; // Geo Scale//
...
},{"d3":"../node_modules/d3/index.js","topojson":"../node_modules/topojson/index.js","Scrubber":"../node_modules/Scrubber/index.js","/data/westvirginiaCounties.json":"data/westvirginiaCounties.json"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
},{}]},{},["../node_modules/parcel-bundler/src/builtins/hmr-runtime.js","scripts/wvCovid-simulation.js"], null)
//# sourceMappingURL=/wvCovid-simulation.57c16ecb.js.map