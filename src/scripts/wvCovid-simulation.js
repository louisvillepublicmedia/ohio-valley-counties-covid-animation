...
function mouseOver(d, i) {
  ...
  tooltip
    ...
    .html(
      `<div class='row'><b>${
        d.properties.NAME20} County</div>`
    )
...
function ready([json, raw]) {
  ...
  d3.select('#deaths-layer').on('click', function(){
    ...
    function deathsMap() {
      map.transition().attr('fill', d =>
        getFilteredData().get(d.properties.GEOID20)
          ? d3.interpolateBlues(Math.log(+getFilteredData().get(d.properties.GEOID20).deaths)/Math.log(10) / 5) : 'white'
      )
    }
    deathsMap()     
    // Make a new scrubber
